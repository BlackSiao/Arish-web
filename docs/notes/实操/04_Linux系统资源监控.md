...
| 字段          | 含义说明                                                                 |
|---------------|--------------------------------------------------------------------------|
| **available** | 最重要！表示系统当前**真正还能分配给新进程的内存** = 空闲内存 free + 可回收的 cache/buffer |
...
## 2. vmstat —— Virtual Meomory Statistics
使用该命令可以对操作系统的虚拟内存、进程、CPU活动进行监控，其提供了对系统的整体情况进行分析的能力
...
### 重要字段快速判断法则
<span style="color: red;">*扩展*: 可以通过 `lscpu` 命令查看服务器有几核</span>
...
| free    | 空闲物理内存大小               |                         |
| buff    | 用作缓冲的内存大小             |                          |
| Cache    | 用作缓存的Cache大小             |  Cache值越大，说明命中Cache的任务越多，磁盘的IO压力就越小                         |
| si/so| Swap In / Swap Out，磁盘的写入写出          | 越大，说明内存越不够用，需要从磁盘交换数据。如果这两个值长期大于0，就会导致CPU资源空闲，等待磁盘IO         |
| bi/bo| 磁盘的块设备读/写（KB/s）                  | 持续很高 + wa 很高：磁盘 I/O 瓶颈，块越大，说明CPU等待IO约久                    |
| us   | 用户进程占CPU运行时间的百分比          |                                                      |
| sy   | 内核进程占CPU运行时间的百分比       |                                                      |
| id   | CPU空闲时间                             |                                                      |
...
在使用top的时候会看到一排信息
```
load average: 0.40, 0.47, 0.38
```
分别代表了最近1分钟、5分钟、15分钟内处于「可运行状态（R）」和「不可中断睡眠状态（D）」的进程平均数量。

- 「可运行状态（R）」:包括正在使用CPU的进程、等待CPU调度的进程
- 「不可中断睡眠状态（D）」:包括等待磁盘 IO | 等待网络 IO | 等待块设备等进程

这也是很常见的情况: CPU很低，但是系统负载很高，说明目前系统卡在其他资源IO上
