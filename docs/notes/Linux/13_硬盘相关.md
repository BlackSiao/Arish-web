...
## 2.使用smartctl命令
为了判断硬盘是否又在正常使用，或者有时候我们想确认一下硬盘的性能参数，用的最多的就是smartctl命令
...
需要特别注意的一点，当一个VG卷内有任何一个盘坏了，由于LVM的全局元数据锁机制，只要卷组内有一个PV断连，就会导致整个VG卷
的元数据陷入"不一致" 或 "挂起" 的情况。
...
```
dmesg -T | grep -i -E "error|io|ext4|xfs"
```

如果UUID没了/文件系统坏了
```
ext4 系统急救： fsck.ext4 -y /dev/mapper/xxx
xfs 系统急救： xfs_repair /dev/mapper/xxx
```
