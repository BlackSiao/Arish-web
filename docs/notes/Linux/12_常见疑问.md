...
有时候在问AI一些技术问题等待时候，AI给出的Linux调试命令里面经常就有这个作为后缀，
这是一个非常常用的错误输出重定向符号。比如说你尝试使用find命令从根目录下遍历去查
看某一个文件，结果一级一级下去，找不到就报一次错，看起来很不爽，就可以在find命令
后加上这个，这样就只有最后找到的文件了。

“2>/dev/null” 是 shell 的错误输出重定向，拆解一下：
  - 2 — 文件描述符 2，即 stderr（标准错误输出）
  - > — 重定向
  - /dev/null — Linux 的"黑洞"设备，写入的内容全部丢弃

所以 2>/dev/null 的意思是：把错误信息丢掉，不显示在屏幕上。
...
```
dmesg -T | grep -v 'deprecated SCSI ioctl' | tail -n 400
```
有时候看日志的时候，会反复被某一行重复报错的无关信息，扰乱查日志的速度，通过-v可以反向先过滤一道
...
在事先不熟悉对应日志的输出信息之前，可以通过 -i 来实现模糊搜索
