...
- 如果中间跳出现丢包但终点无丢包，通常是中间设备对 ICMP 限速（可忽略）；若终点也丢包，才为真实链路问题。
...
### 3.4 显示本机到某个网段之间TCP连接情况

```
ss -tni dst 21.42.57.0/24
```

### 3.4 显示目前和本机连接最多的前20个IP

```
ss -tan | awk '{print $5}' | cut -d: -f1 | sort | uniq -c | sort -rn | head -20
```
第一行看到 " 285 ["这样的情况，是正常的,表明的是 IPv6 地址被截断了。
...
```查看网卡是否有丢包/报错(历史数据)
ip -s link show
# 主要是查看 RX/TX 的 errors|dropped|overrun
```
...
```设置数据包的大小
ping -M do -s 1428 [目标IP]
---
完整包大小的计算: 1428 (payload) +  8 (ICMP 头) + 20 (IP 头) = 1456 字节

```
在某些特殊情况下，路由器会修改自己的mtu大小,对过大或者过小的数据包进行丢弃，我们可以通过 -s 来强制设置探测数据包的大小，来判断是否丢包是因为MTU设置的
