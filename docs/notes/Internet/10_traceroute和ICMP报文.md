...
Traceroute测试出来的是某一时刻，从 测试的发起端 <---> 测试的终端 之间的网络链路联通情况，但整体的网络链路是动态的，我们就需要观察动态的网络波动，这个时候就可以使用 MTR 命令来动态的查看 网络链路 在一段时间内容的网络质量

```最常用的命令格式
mtr -wzbc 100 [目标机器IP]
    -w: 宽格式输出，一行显示完
    -z: 每一跳都会显示对应的ASN
    -b: 同事显示主机名和IP （MTR默认会自动DNS成域名的）
    -c 100 : 每一跳发送100个探测包，统计丢包/延迟
```
